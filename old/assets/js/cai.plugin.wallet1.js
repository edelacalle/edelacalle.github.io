const WalletStatus = {'guest':0 , 'wallet':1 , 'register':2 , 'data':3}
const IconStatus = { 0:'bi bi-question'  , 1:'bi bi-wallet' , 2:'bi bi-person-badge-fill'  , 'data':'bi bi-person'}
const ColorStatus = { 0:'btn btn-danger'  , 1:'btn btn-info' , 2:'btn btn-success'  , 'data':'btn btn-danger'}
...
;(function($) {
    ...
    $.wallet = function(element, options) {
        ...
        plugin.import_json = async function(e) {
            console.log("import eeee", e.files)
        ...
        plugin.update =  async function() {
            ...
            var cText = (this.settings.status == WalletStatus.guest)? 'Sin Pasaporte': 
                (this.settings.status == WalletStatus.wallet ) ? this.abbr() :
                (this.settings.status == WalletStatus.register && app.user ) ?  app.user.nomalias :''
            ...
            this.settings.$w.append(
                $('<button/>')
                  ...
                  .addClass(ColorStatus[this.settings.status])
                 // .append('<i/>')
                  .addClass(IconStatus[this.settings.status])
                 // .addClass('pepeillo')
        ...
        plugin.abbr =  () => (plugin.settings.wallet) ? plugin.settings.wallet.address.substring(0,5) + '...' + plugin.settings.wallet.address.substring(38) : "";
        plugin.addr = () => (plugin.settings.wallet)?plugin.settings.wallet.address:'';
